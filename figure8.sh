...
for packet_loss_ratio in 0 0.02 0.04 0.06 0.08 0.1; do
  ...
  screen -dmS mininet -L -Logfile logs/screen_mininet_$packet_loss_ratio.log python3 -m network.SimulatedNetwork --num-switches=$num_switches --random-type=erdos-renyi --erdos-renyi-prob=0.5 --loss-switch-ratio=$loss_switch_ratio --packet-loss-ratio=$packet_loss_ratio --iperf3-interval=0 --num-bytes-sent=100000 --bitrate=100M
  sleep 15
  ...
  screen -XS controller quit
  sleep 5
  mv stats/iperf3_stats.json stats/iperf3_stats_$packet_loss_ratio.json
